// 1. Multi-Color Simultaneous Typing Animation
    ...
    (function () {
      ...
      function parseRelease(data) {
        if (!data) return null;
        // GitHub official REST format
        if (Array.isArray(data.assets)) {
          const dmg = data.assets.find(a => a.name && a.name.toLowerCase().endsWith('.dmg'))
            || data.assets.find(a => a.browser_download_url && a.browser_download_url.toLowerCase().endsWith('.dmg'))
            || data.assets[0];
          if (dmg && dmg.browser_download_url) {
            return {
              url: dmg.browser_download_url,
              name: dmg.name || dmg.browser_download_url.split('/').pop(),
              tag: data.tag_name || data.name || ''
            };
          }
        }
        // Ungh.cc format: { release: { tag: "...", assets: [ ... ] } }
        const release = data.release || data;
        if (release && Array.isArray(release.assets)) {
          const dmg = release.assets.find(a => a.downloadUrl && a.downloadUrl.toLowerCase().endsWith('.dmg'))
            || release.assets.find(a => a.name && a.name.toLowerCase().endsWith('.dmg'))
            || release.assets[0];
          const url = dmg ? (dmg.downloadUrl || dmg.browser_download_url) : null;
          if (url) {
            return {
              url: url,
              name: dmg.name || url.split('/').pop(),
              tag: release.tag || release.name || ''
            };
          }
        }
        return null;
      ...
      async function fetchFromEndpoint(url, timeoutMs = 3500) {
        ...
        try {
          ...
          if (!parsed || !parsed.url) throw new Error('No valid dmg asset found');
          return parsed;
        } catch (err) {
          clearTimeout(timer);
          throw err;
      ...
      // Query latest release from all sources (race & fallback)
      function fetchLatestRelease(force = false) {
        if (!force && activeFetchPromise) return activeFetchPromise;

        activeFetchPromise = Promise.any(
          endpoints.map(ep => fetchFromEndpoint(ep))
        ).then(result => {
          latestReleaseInfo = result;
          updateButtons(result);
          return result;
        }).catch(async (err) => {
          console.warn('[Download] 并行获取最新安装包失败，依次重试:', err);
          for (const ep of endpoints) {
            try {
              const res = await fetchFromEndpoint(ep, 4000);
              if (res && res.url) {
      ...
      function updateButtons(info) {
        ...
        const buttons = document.querySelectorAll('.js-download-btn');
        buttons.forEach(btn => {
          btn.href = info.url;
          btn.setAttribute('download', info.name);
          btn.title = `点击直接下载 ${info.tag} (${info.name})`;
        });
        console.log(`[Download] 成功获取最新安装包: ${info.tag} - ${info.name}`);
      }
      ...
      downloadButtons.forEach(btn => {
        btn.addEventListener('pointerenter', () => {
          if (!latestReleaseInfo) fetchLatestRelease();
        }, { passive: true });
        ...
        btn.addEventListener('click', async (e) => {
          ...
          try {
            ...
            const downloadUrl = (latest && latest.url) ? latest.url : btn.href;
            const fileName = (latest && latest.name) ? latest.name : (btn.getAttribute('download') || 'ShotMock.dmg');

            if (downloadUrl && !downloadUrl.includes('javascript:') && !downloadUrl.includes('#')) {
            ...
            console.error('[Download] 获取最新版本失败:', err);
            if (btn.href && !btn.href.includes('javascript:') && !btn.href.includes('#')) {
